...
my_data <- dplyr::bind_rows(pitfalls, transects, quadrats, audiomoth_birds, audiomoth_bats) |>
  dplyr::mutate(
    parentEventID = paste("DIGITAF_FLANDERS", "LOCATION", Loc, sep = ":"),
...
event <- my_data |>
  ...
  dplyr::mutate(
    ...
    samplingEffort = dplyr::recode_values(
      .data$samplingProtocol,
      "pitfall" ~ "1 trap for 14 days, Ø 9 cm, depth 11 cm, antifreeze",
      "15' transect" ~ "15 min over 50 m",
      "10m² quadrat" ~ "10 m² quadrat",
      "audiomoth recording" ~ 
...
occurrence <-      
  ...
  dplyr::mutate(
    ...
    datasetID = dataset_id,
    datasetName = title,
    basisOfRecord = dplyr::recode_values(
      .data$Spec_group,
      "birds" ~ "MachineObservation",
      "bats" ~ "MachineObservation",
      "plants" ~ "HumanObservation",
      "lepidoptera" ~ "HumanObservation",
      default = "materialSample"
    ),
    occurrenceID = .data$occurrenceID, 
    organismQuantity = .data$Number, 
    organismQuantityType = dplyr::recode_values(
      .data$Spec_group,
    ...
    organismQuantityType = dplyr::recode_values(
      ...
      "birds" ~ NA_character_,
      "bats" ~ NA_character_,
      default = .data$organismQuantityType
    ),
